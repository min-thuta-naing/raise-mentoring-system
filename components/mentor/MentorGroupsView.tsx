    ...
    if (myGroups.length === 0) {
        return (
            <div className="flex flex-col items-center justify-center py-20 bg-white rounded-[3rem] border border-dashed border-gray-200 animate-fade-in shadow-2xl shadow-indigo-900/5">
                ...
                <p className="text-sm text-gray-500 mt-2">You haven't been assigned to any mentoring groups yet.</p>
            </div>
        );
    }
                    ...
                    <div key={group.id} className="bg-white rounded-[2.5rem] shadow-2xl shadow-indigo-900/5 border border-gray-100 overflow-hidden transition-all duration-500 hover:shadow-indigo-900/10">
                        ...
                        <div className="p-8 flex flex-col md:flex-row justify-between items-center gap-6">
                            <div className="flex items-center gap-6">
                                <div className="w-16 h-16 bg-emerald-600 rounded-3xl shadow-xl shadow-emerald-600/30 flex items-center justify-center text-white shrink-0">
                                    <Hash size={32} />
                                </div>
                                <div className="min-w-0">
                                    <div className="flex items-center gap-2 mb-1">
                                        <span className="text-[10px] font-black bg-emerald-50 text-emerald-600 px-3 py-1 rounded-full uppercase tracking-widest whitespace-nowrap">
                                    ...
                                    <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-1 flex items-center gap-2">
                                        <Users size={12} /> {group.studentIds.length} Students • {group.mentorIds.length} Mentors
                                    </p>
                                </div>
                            </div>
                            ...
                            <div className="flex items-center gap-4 w-full md:w-auto">
                                <button 
                                    ...
                                    className="relative flex-1 md:flex-none px-8 py-4 bg-emerald-600 text-white font-black uppercase text-[10px] tracking-[0.2em] rounded-2xl shadow-xl shadow-emerald-600/20 hover:bg-emerald-700 transition-all flex items-center justify-center gap-3 active:scale-95"
                                >
                                ...
                                <button 
                                    ...
                                    className={`p-4 rounded-2xl transition-all border-2 flex items-center justify-center shrink-0 ${
                                        isExpanded 
                                            ...
                                            : 'bg-white border-gray-100 text-emerald-600 hover:border-emerald-100'
                                    }`}
                            ...
                            <div className="p-8 bg-gray-50/30 space-y-10">
                                ...
                                <div className="space-y-6">
                                    <div className="flex items-center gap-3 px-2">
                                        ...
                                        <h2 className="text-sm font-black uppercase tracking-widest text-gray-900">Co-Mentors</h2>
                                    </div>
                                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                                        {members.mentors.length > 0 ? members.mentors.map(mentor => (
                                            <div key={mentor.id} className="bg-white p-4 rounded-[1.5rem] border border-gray-100 shadow-lg shadow-emerald-900/5 flex items-center gap-4 group hover:border-emerald-200 transition-colors">
                                                ...
                                                <div className="min-w-0">
                                                    ...
                                                    <p className="text-[9px] font-bold text-emerald-500 uppercase tracking-[0.1em] mt-0.5">Partner Mentor</p>
                                                </div>
                                            </div>
                                        )) : (
                                            <p className="text-xs text-gray-400 italic px-2">No other mentors assigned to this group.</p>
                                        )}
                                    </div>
                                </div>
                                ...
                                <div className="space-y-6">
                                    <div className="flex items-center gap-3 px-2">
                                        ...
                                        <h2 className="text-sm font-black uppercase tracking-widest text-gray-900">Mentored Students</h2>
                                    </div>
                                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                                        {members.students.map(student => (
                                            <div key={student.id} className="bg-white p-4 rounded-[1.5rem] border border-gray-100 shadow-lg shadow-emerald-900/5 flex items-center gap-4 hover:bg-emerald-50/50 transition-all group">
                                                <div className="relative">
                                                    <img src={student.avatarUrl} className="w-12 h-12 rounded-xl border-2 border-white shadow-md object-cover" alt="" />
                                                    <div className="absolute -bottom-0.5 -right-0.5 w-3 h-3 bg-green-500 border-2 border-white rounded-full"></div>
                                                </div>
                                                <div className="min-w-0 flex-1">
                                                    <h3 className="font-black text-gray-800 uppercase tracking-tight truncate text-sm">{student.fullName}</h3>
                                                    <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest mt-0.5">Student</p>
                                                </div>
                                            </div>
                                        ))}
                                    </div>
