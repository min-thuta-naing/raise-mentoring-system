...
export const AuditLogModal: React.FC<AuditLogModalProps> = ({ log, users, onClose }) => {
  return (
    <div className="fixed inset-0 bg-[#0a0a0f]/60 z-50 flex items-center justify-center p-4 backdrop-blur-md animate-in fade-in duration-300">
      <div className="bg-white rounded-[1.5rem] shadow-2xl max-w-lg w-full overflow-hidden animate-in zoom-in-95 duration-300">
        ...
        <div className="p-4 max-h-[400px] overflow-y-auto">
          <div className="relative border-l-2 border-[#454040]/10 ml-3 space-y-6">
            {log.history.map((h, idx) => {
              const actor = users.find(u => u.id === h.actorId);
              return (
                <div key={idx} className="relative pl-6">
                  <div className="absolute -left-[9px] top-0 w-4 h-4 rounded-full bg-[#454040] border-2 border-white"></div>
